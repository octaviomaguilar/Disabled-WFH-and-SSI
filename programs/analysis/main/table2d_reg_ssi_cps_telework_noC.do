...
*merge in cps telework data: 
merge m:1 occ2 year using "$ipums/clean/cps_telework_noC.dta", keep(3) nogen
...
*2.1: OLS regressions: 
reg incssi cps_telework if disability_noC == 1 [aw=wtfinl], robust
estadd local fe No
...
*2.2: adding controls
reg incssi cps_telework i.female i.married i.hispanic i.race i.famsize age child incdivid incrent incasist employed incvet incsurv delta_wage delta_collects_SSI if disability_noC == 1 [aw=wtfinl], robust
estadd local fe No
...
*2.3: adding FE 
reghdfe incssi cps_telework i.female i.married i.hispanic i.race i.famsize age child incdivid incrent incasist employed incvet incsurv delta_wage delta_collects_SSI if disability_noC == 1 [aw=wtfinl], absorb(Istate_month Inaics2_month Imonth)
estadd local fe Yes
estadd local controls Yes
estadd local IV No
...
/* Second Stage */ 
*IV No controls 
ivregress 2sls incssi (cps_telework=remote_iv1) if disability_noC == 1 [aw=wtfinl], robust
estadd local fe No
estadd local controls No
estadd local IV Yes
...
*IV with controls
ivregress 2sls incssi (cps_telework=remote_iv2) if disability_noC == 1 [aw=wtfinl], robust
estadd local fe No
estadd local controls Yes
estadd local IV Yes
...
ivregress 2sls incssi (cps_telework=remote_iv3) if disability_noC == 1 [aw=wtfinl], robust
estadd local fe Yes
estadd local controls Yes
estadd local IV Yes
